...
class DecimalsLessonPage extends StatefulWidget{ 
  ...
  const DecimalsLessonPage({required this.pdfPath, required this.lessonNum, Key? key}) : super(key: key);
  ...
  void decimalsLessons(){
    Navigator.pushReplacement( 
      context, PageRouteBuilder(
        pageBuilder: (context, animation, secondaryAnimation) => DecimalsLessons(), 
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          const begin = Offset(-1.0, 0.0);
          const end = Offset.zero;
          const curve = Curves.easeInOut;
          ...
          children: [
            Expanded(
              child: Container( //Screen borders for the background color
                ...
                child: Stack(
                  children: [
                    backTextMenuBar(context, decimalsLessons, "Lesson ${widget.lessonNum}"),
                    Align(
                      alignment: Alignment(0.0, 0.8),
                      child: Container(
                        width: 1042,
